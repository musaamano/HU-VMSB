...
import './FuelStationSettings.css';

const FuelStationSettings = () => {
  const [activeTab, setActiveTab] = useState('account');
  const [message, setMessage] = useState({ type: '', text: '' });
  const [profileDisplay, setProfileDisplay] = useState({
    name: 'Ahmed Hassan',
    email: 'ahmed.hassan@hu.edu.et',
  ...
  const saveAllSettings = () => {
    ...
    localStorage.setItem('fuelStationSettings', JSON.stringify(allSettings));
  ...
  const handleSaveAccount = () => {
    ...
    const event = new CustomEvent('accountSettingsUpdated', {
    ...
    window.dispatchEvent(new StorageEvent('storage', {
      key: 'fuelStationSettings',
      newValue: localStorage.getItem('fuelStationSettings')
    }));
  ...
  const handleExportSettings = () => {
    ...
    link.href = url;
    link.download = `fuel-station-settings-${new Date().toISOString().split('T')[0]}.json`;
    link.click();
  ...
  const handleResetSettings = () => {
    if (window.confirm('Are you sure you want to reset all settings to defaults? This action cannot be undone.')) {
      localStorage.removeItem('fuelStationSettings');
  ...
  return (
    <div className="fuel-settings-container">
      <div className="fuel-settings-header">
      ...
      {message.text && (
        <div className={`fuel-settings-message ${message.type}`}>
          <span className="message-icon">{message.type === 'success' ? '✓' : '⚠'}</span>
          <span>{message.text}</span>
        </div>
      )}

      <div className="fuel-settings-content">
        <div className="fuel-settings-tabs">
          <button className={`fuel-settings-tab ${activeTab === 'account' ? 'active' : ''}`} onClick={() => setActiveTab('account')}>
            <span className="tab-icon">👤</span> Account
          </button>
          <button className={`fuel-settings-tab ${activeTab === 'password' ? 'active' : ''}`} onClick={() => setActiveTab('password')}>
            <span className="tab-icon">🔒</span> Password
          </button>
          <button className={`fuel-settings-tab ${activeTab === 'security' ? 'active' : ''}`} onClick={() => setActiveTab('security')}>
            <span className="tab-icon">🛡️</span> Security
          </button>
          <button className={`fuel-settings-tab ${activeTab === 'notifications' ? 'active' : ''}`} onClick={() => setActiveTab('notifications')}>
            <span className="tab-icon">🔔</span> Notifications
          </button>
          <button className={`fuel-settings-tab ${activeTab === 'system' ? 'active' : ''}`} onClick={() => setActiveTab('system')}>
            <span className="tab-icon">💻</span> System
          </button>
          <button className={`fuel-settings-tab ${activeTab === 'privacy' ? 'active' : ''}`} onClick={() => setActiveTab('privacy')}>
            <span className="tab-icon">🔐</span> Privacy
          </button>
        </div>

        <div className="fuel-settings-panel">
          {activeTab === 'account' && (
            <div className="fuel-settings-section">
              <div className="section-header">
                <h2>Account Information</h2>
                <p>Update your personal and professional details</p>
              </div>
              <form className="fuel-settings-form">
                <div className="form-grid">
                  ...
                  <div className="form-group">
                    ...
                    <input type="text" name="fuelStationName" value={accountSettings.fuelStationName} onChange={handleAccountChange} />
                  </div>
                  <div className="form-group">
              ...
              <form className="fuel-settings-form">
                ...
                <div className="notification-types">
                  ...
                  <div className="security-options">
                    <div className="security-option">
                      <div className="option-info">
                        <h4>Fuel Request Alerts</h4>
                        <p>New fuel requests and approvals</p>
                      </div>
                      <label className="toggle-switch">
                        <input type="checkbox" name="fuelRequestAlerts" checked={notificationSettings.fuelRequestAlerts} onChange={handleNotificationChange} />
                        <span className="toggle-slider"></span>
                      </label>
                    </div>
                    <div className="security-option">
                      <div className="option-info">
                        ...
                        <p>Alerts when fuel inventory is running low</p>
                      </div>
                      <label className="toggle-switch">
                        <input type="checkbox" name="lowInventoryAlerts" checked={notificationSettings.lowInventoryAlerts} onChange={handleNotificationChange} />
                        <span className="toggle-slider"></span>
                      </label>
                    </div>
                    <div className="security-option">
      ...
      <div className="fuel-settings-footer">
...
export default FuelStationSettings;