...
router.post('/requests', ...guard, async (req, res) => {
  ...
  // Notify all Transport Officers about the new request
  const transportOfficers = await User.find({ role: 'TRANSPORT', isActive: true }).select('_id');
  if (transportOfficers.length > 0) {
    await Notification.insertMany(transportOfficers.map(officer => ({
