...
router.put('/complaints/:id/respond', ...guard, async (req, res) => {
  ...
  if (action === 'approve') {
    // Admin approves the complaint - move to under_review
    statusUpdate = { 
      status: 'under_review',
    ...
    notificationData = {
      ...
      message: `Your complaint regarding "${complaint.category}" has been approved and is now under review.`,
      severity: 'normal'
    };
  } else if (action === 'respond') {
    // Admin provides response - move to responded
    statusUpdate = { 
      status: 'responded',
      response: response,
      respondedBy: req.user._id,
    ...
    notificationData = {
      ...
      message: `Your complaint regarding "${complaint.category}" has received a response.`,
      severity: 'normal'
    };
  } else if (action === 'resolve') {
    // Admin resolves the complaint - move to resolved
    statusUpdate = { 
      status: 'resolved',
      response: response,
      respondedBy: req.user._id,
    ...
    notificationData = {
      ...
      message: `Your complaint regarding "${complaint.category}" has been resolved.`,
      severity: 'low'
    };
  } else {
    // Default behavior - just respond
    statusUpdate = { 
      response: response, 
      status: 'responded', 
      respondedBy: req.user._id, 
...
// Separate approval endpoint for clarity
router.put('/complaints/:id/approve', ...guard, async (req, res) => {
  const complaint = await Complaint.findByIdAndUpdate(
    req.params.id,
    { status: 'under_review', respondedBy: req.user._id, respondedAt: new Date() },
    { new: true }
  );
